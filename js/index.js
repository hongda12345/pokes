...
* @Last Modified by:   宏达
* @Last Modified time: 2017-10-18 15:26:36
*/
$(function(){
	...
	for(;index<poke.length;index++){
		$('<div>').addClass('poke zuo').attr('id',`${-5}_${-5}`).data('num',poke[index].num).css('background-image',`url(img/${poke[index].num}${poke[index].hua}.png)`).appendTo('.desk').delay(index*10).animate({left:5,top:460,opacity:1})
	}
	console.log(poke)
    ...
    btnL.on('click',function(){
    	...
    	$('.you').each(function(index){
            $(this).css('zIndex',zindex++).delay(index*100).animate({top:460,left:5}).removeClass('you').addClass('zuo');
    	})
    })
